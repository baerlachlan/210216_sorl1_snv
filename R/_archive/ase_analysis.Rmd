...
metadata <- read_csv(here("files/SraRunTable.txt")) %>%
  ...
  mutate(
    Genotype = factor(Genotype, levels = c(
      "sorl1R122Pfs/+", "sorl1V1482Afs/+", "sorl1V1482Afs/R122Pfs", "wild-type"
    )),
    alias = c(
...
Differential expression results previously analysed by Karissa were loaded as a list of `topTable`s.

```{r, include=FALSE, eval=FALSE}
# dgeList <- readRDS(here("files/geneDGE_TMM_filteredlowly_RUVk1.rds"))
# dgeList$samples <- mutate(
#   dgeList$samples,
#   Genotype = fct_recode(Genotype, "Trans" = "trans")
# )
# design <- model.matrix(
#   ~ Genotype + Sex + Tank + W_1,
#   data = dgeList$samples
# ) %>% 
#   set_colnames(gsub("Genotype", "", colnames(.)))
# fit <- estimateDisp(dgeList, design) %>% 
#   glmFit()
# topTables <- colnames(design)[2:4] %>%
#   sapply(function(x){
#     glmLRT(fit, coef = x) %>%
#       topTags(n = Inf) %>%
#       .[["table"]] %>%
...
#       arrange(PValue) %>%
#       dplyr::select(
#         gene_id, symbol, logFC, logCPM, PValue, FDR, everything()  
#       ) %>%
#       mutate(
#         coef = x,
...
```{r}
nDE <- sapply(topDE, function(x){
  dplyr::filter(x, DE) %>%
    nrow()
})
```

All 3 mutant genotypes were tested for differential gene expression compared to the wild-type genotype.
The `r names(nDE[1])` genotype showed `r nDE[1]` DE genes, the `r names(nDE[2])` genotype showed `r nDE[2]` DE genes, and the `r names(nDE[3])` genotype showed `r nDE[3]` DE genes.

### Allele specific expression

Allele specific expression was tested at the gene-level with `GeneiASE` software.

```{r}
files <- list.files(
  "/hpcfs/users/a1647910/210216_sorl1_snv/11_geneiase/2_ase",
  full.names = TRUE
)
