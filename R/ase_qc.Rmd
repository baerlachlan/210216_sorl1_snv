...
These large proportions were noted as a concern for downstream processing due to a substantial loss of information when duplicates are excluded.
Such large estimates are likely due to the conservative nature of the duplicate detection strategy, which is hard to avoid when a dataset consists of single-end reads, and therefore the software can only classify dulplicates based on a single position.
...
`ApplyBQSR` uses the recalibration file to go through the reads and adjust each base's score based on what bins it falls in.
...
```{r}
files <- list.files(
  "/hpcfs/users/a1647910/210216_sorl1_snv/05_addRG/metrics",
  full.names = TRUE
)
...
```{r}
files <- list.files(
  "/hpcfs/users/a1647910/210216_sorl1_snv/07_recalBases/metrics",
  full.names = TRUE
)
...
```{r}
files <- list.files(
  "/hpcfs/users/a1647910/210216_sorl1_snv/07_recalBases/recal",
