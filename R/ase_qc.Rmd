...
`Picard MarkDuplicates` tool classifies duplicates by comparing sequences in the 5 prime position of reads and read pairs in a `BAM` file.
This is indeed a conservative approach as there remains the possibility of marking non-technical duplicates that appear to be duplicates by chance.
...
These large proportions are likely due to the conservative nature of the duplicate detection strategy.
Additionally, this dataset consists of single-end reads, and therefore the software can only classify dulplicates based on a single sequence.
...
The intervals were also exported as a `BED` file for use with other software.

```{r}
bedPath <- "/hpcfs/users/a1647910/210216_sorl1_snv/08_callSnvs/intervals/exons.bed"
...
Base quality score recalibration (BQSR) is a recommended technique in the `GATK1 workflow used to assign accurate confidence scores to each sequenced base.
Real data contains systematic biases due to artifacts of sample preparation, sequencing and mapping.
BQSR aims to correct for the range of these types of artifacts, as the downstream tools rely on base qualities to weigh the variant calls they make.

```{r}
tmp <- read_csv("/hpcfs/users/a1647910/210216_sorl1_snv/07_recalBases/recal/SRR11951228.analyzeCovariates.csv")
tmp %>%
  dplyr::filter(CovariateName == "QualityScore") %>%
  ggplot(aes(AverageReportedQuality, EmpiricalQuality)) +
  geom_point(aes(colour = Recalibration)) +
...
tmp %>%
  ...
  mutate(CovariateValue = factor(CovariateValue, levels = unique(CovariateValue))) %>%
...
calledMetrics <- list.files(
  ...
  path = "/hpcfs/users/a1647910/210216_sorl1_snv/08_callSnvs/1_called/log/",
  pattern = "detail",
  full.names = TRUE
) %>%
...
extractedMetrics <- list.files(
  ...
  path = "/hpcfs/users/a1647910/210216_sorl1_snv/08_callSnvs/2_extracted/log/",
  pattern = "detail",
  full.names = TRUE
) %>%
  lapply(read_tsv, comment = "#") %>%
...
filteredMetrics <- list.files(
  ...
  path = "/hpcfs/users/a1647910/210216_sorl1_snv/08_callSnvs/3_filtered/log/",
  pattern = "detail",
  full.names = TRUE
) %>%
  lapply(read_tsv, comment = "#") %>%
...
selectedMetrics <- list.files(
  ...
  path = "/hpcfs/users/a1647910/210216_sorl1_snv/08_callSnvs/4_selected/log/",
  pattern = "detail",
  full.names = TRUE
) %>%
  lapply(read_tsv, comment = "#") %>%
...
`GATK`'s initial variant calling output provides a number of different types of variants, for example indels, single nucleotide and multi nucleotide variants.
The total number of all types of variants ranges between `r pander(range(filtProgress$initial))`.
...
A filter was applied such that only biallelic SNVs remained for ASE analysis.
The number of biallelic SNVs ready for ASE analysis ranged between `r pander(range(filtProgress$biallelic))`.
...
```{r, results='hide'}
vcfPaths <- list.files(
  "/hpcfs/users/a1647910/210216_sorl1_snv/08_callSnvs/4_selected",
  pattern = ".vcf.gz$",
  full.names = TRUE
)
...
```{r}
gdsPaths <- list.files(
  "/hpcfs/users/a1647910/210216_sorl1_snv/08_callSnvs/5_gds",
  pattern = ".gds$",
  full.names = TRUE
)
```
...
lapply(snvList, function(x){
  ...
  lapply(chrList, function(x){
    ...
    path <- paste0(
      "/hpcfs/users/a1647910/210216_sorl1_snv/09_wasp/1_snvs/",
      sample,
      "/",
      chr,
      ".snps.txt.gz"
...
lapply(seq_along(staticCounts), function(x){
  path <- paste0(
    "/hpcfs/users/a1647910/210216_sorl1_snv/11_geneiase/1_counts/",
