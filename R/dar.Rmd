---
title: "DAR Analysis"
subtitle: "210216_sorl_snv dataset"
author: "Lachlan Baer"
...
metadata <- read_csv(here("files/SraRunTable.txt")) %>%
  ...
  dplyr::select(
    sample = Run, genotype, genotype2 = Genotype, alias, gender, tank = Tank, group
  )
```
...
```{r vcf}
vcf_path <- file.path(
  "/hpcfs/users/a1647910/210216_sorl1_snv",
...
```{r cumDAR_V1482Afs_het}
plotCumDAR(DAR$V1482Afs_het, "15", "V1482Afs_het vs. WT")
```

```{r cumDAR_R122Pfs_het}
plotCumDAR(DAR$R122Pfs_het, "15", "R122Pfs_het vs. WT")
```

```{r cumDAR_Trans}
plotCumDAR(DAR$Trans, "15", "Trans vs. WT")
```
...
```{r winRanges_V1482Afs_het}
plotWinRanges(winRanges$V1482Afs_het, "V1482Afs_het vs WT")
```

```{r winRanges_R122Pfs_het}
plotWinRanges(winRanges$R122Pfs_het, "R122Pfs_het vs WT")
```

```{r winRanges_Trans}
plotWinRanges(winRanges$Trans, "Trans vs WT")
```
...
```{r DAR_V1482Afs_het_mutant}
plotChrDAR(
  DAR = DAR$V1482Afs_het,
  chromosome = "15",
  features = genes[genes$gene_id %in% deGenes$V1482Afs_het],
  foi = genes[genes$gene_name == "sorl1"]
)
```

And comparing this to a non-mutant chromosome.

```{r DAR_V1482Afs_het_non-mutant}
plotChrDAR(
  DAR = DAR$V1482Afs_het,
  chromosome = "3",
  features = genes[genes$gene_id %in% deGenes$V1482Afs_het],
...
```{r DAR_R122Pfs_het_mutant}
plotChrDAR(
  DAR = DAR$R122Pfs_het,
  chromosome = "15",
  features = genes[genes$gene_id %in% deGenes$R122Pfs_het],
  foi = genes[genes$gene_name == "sorl1"]
)
```
...
plotChrDAR(
  DAR = DAR$Trans,
  chromosome = "15",
  features = genes[genes$gene_id %in% deGenes$Trans],
  foi = genes[genes$gene_name == "sorl1"]
)
```
