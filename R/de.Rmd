...
topTables[[1]] %>%
  ...
  kable(
    align = "l",
    caption = paste(
      "The", nrow(.), "differentially expressed genes for V1482Afs_het genotype",
...
topTables[[2]] %>%
  ...
  kable(
    align = "l",
    caption = paste(
      "The", nrow(.), "differentially expressed genes for R122Pfs_het genotype",
      "in comparison to wildtype"
    )
  ) %>%
  kable_styling(
...
topTables[[3]] %>%
  ...
  kable(
    align = "l",
    caption = paste(
      "The", nrow(.), "differentially expressed genes for Trans genotype",
      "in comparison to wildtype"
    )
  ) %>%
  kable_styling(
...
fit_cqn <- estimateDisp(dgeList, design) %>%
  glmFit()
topTables_cqn <- colnames(design)[2:4] %>%
  sapply(function(x){
...
```{r}
comp_samples <- lapply(c("(V1482Afs_het|WT)", "(R122Pfs_het|WT)"), function(comp){
  metadata %>%
    dplyr::filter(str_detect(genotype, comp)) %>%
    pull(sample)
}) %>%
  set_names(c("V1482Afs_het", "R122Pfs_het"))
```

```{r}
deGenes <- lapply(topTables_chosen[1:2], function(x){
  dplyr::filter(x, DE) %>%
    pull(gene_id)
})
...
````{r}
manPlots <- lapply(topTables_chosen, function(topTable){
  man <- topTable %>%
    ...
    mutate(
      ...
      colour = ifelse(chromosome == "15", "c", colour)
    )
  axis <- man %>%
    group_by(chromosome) %>%
